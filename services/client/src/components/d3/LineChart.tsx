...
const LineChart = (props: any) => {
  ...
  const addChartLine = (dataset: [any,number][],svg: d3.Selection<null, unknown, null, undefined>,color:string) => {
    var line2 = d3.line()
  ...
  function addChartSymbol(svg: d3.Selection<null, unknown, null, undefined>, dataset: [any,number][], symbolClass:string, hasNoFill: boolean, color:string) {
    
    svg.selectAll(symbolClass)
        .data(dataset)
        .enter().append("circle")
        .attr('r', 5)
  ...
  const drawLineChart = () => {
    ...
    ChartUtils.initChart(g, title,"Time","Portfolio Value ($)", xScale, yScale, width, height, 0, height, margin.top, margin.left);
